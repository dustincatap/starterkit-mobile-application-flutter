...
read -p "Enter project name (e.g. my_app): " PROJECT_NAME
read -p "Enter app id (e.g. com.example.my.app): " APP_ID
...
PROJECT_REPO_NAME=$(echo "$PROJECT_NAME" | sed -E 's/_/-/g')
...
APP_NAME=$(echo "$APP_NAME" | sed -E 's/([A-Z])/ \1/g')
...
echo "Updating secrets with project values..."
sed -i.bak "s/com\.example\.starterkit\.app/$APP_ID/g" "$PROJECT_DIR/.secrets/dev.json" "$PROJECT_DIR/.secrets/prod.json"
sed -i.bak "s/Starterkit App/$APP_NAME/g" "$PROJECT_DIR/.secrets/dev.json" "$PROJECT_DIR/.secrets/prod.json"
rm -f "$PROJECT_DIR/.secrets/"*.bak
...
NEW_MAIN_ACTIVITY_DIR="$ANDROID_PROJECT_DIR/app/src/main/kotlin/$(echo "$APP_ID" | sed -E 's/\./\//g')"
